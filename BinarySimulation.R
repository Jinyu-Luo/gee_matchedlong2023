...
b3_r <- log(1.5) #visit
b4_r <- log(0.9) #age
b5_r <- log(0.8) #sex_female
b6_r <- log(1.2) #bsa baseline
b7_r <- log(1.5) #visit:bav
...
for (s in 1:simnum) {
  ...
  # generate binary outcome values
  bi <- rnorm(N, mean = 0, sd = 5)
  b <- rep(bi, each = maxT)
  e <- rnorm(N*maxT, mean = 0, sd = 1)
  logit_p <- b1_r + b2_r*bav + b3_r * visit + b4_r * age + b5_r * female + b6_r * bsa_bl + b7_r * bav * visit + b + e
  prob <- exp(logit_p) / (1 + exp(logit_p))
  outcome <- rbinom(length(prob), 1, prob)
  
  simdat <- as.data.frame(cbind(id, visit, age, female, bsa_bl, bav, outcome))
  ...
  # ** a) entire cohort----
  #1) independece
  gee_ind <- geeglm(outcome ~ bav + visit + age + female + bsa_bl + bav:visit, 
  ...
  #2) exchangeable
  gee_exch <- geeglm(outcome ~ bav + visit + age + female + bsa_bl + bav:visit, 
                     data = simdat, family = binomial,
  ...
  #3) AR1
  gee_ar1 <- geeglm(outcome ~ bav + visit + age + female + bsa_bl + bav:visit, 
                    data = simdat, family = binomial,
  ...
  #4) unstructured
  gee_unstr <- geeglm(outcome ~ bav + visit + age + female + bsa_bl + bav:visit, 
                      data = simdat, family = binomial,
  ...
  # ** b) sample data----
  #1) independece
  gee_inds <- geeglm(outcome ~  visit + bav:visit, data = matched_long, 
                     family = binomial, id = id, waves = visit, 
                     corstr = "independence")
  ...
  #2) exchangeable
  gee_exchs <- geeglm(outcome ~  visit + bav:visit, data = matched_long, 
                      family = binomial, id = id, waves = visit, 
                      corstr = "exchangeable")
  ...
  #3) AR1
  gee_ar1s <- geeglm(outcome ~  visit + bav:visit, data = matched_long, 
                     family = binomial, id = id, waves = visit, corstr = "ar1")
  ...
  #4) Unstructure
  gee_unstrs <- geeglm(outcome ~  visit + bav:visit, data = matched_long, 
                       family = binomial, id = id, waves = visit, 
                       corstr = "unstructured")
  ...
  lme <- lmer(outcome ~  visit + bav:visit + (1|id), data = matched_long)
